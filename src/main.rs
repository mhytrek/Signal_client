...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Command::DeleteMessage(args) => match(args.contact,args.group){(Some(c),None)=>messages::send::contact::send_delete_message_cli(c,args.timestamp).await?,(None,Some(g))=>messages::send::group::send_delete_message_cli(g,args.timestamp).await?,_=>unreachable!(),},
    }
